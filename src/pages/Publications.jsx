...
const Publications = () => {
  ...
  useEffect(() => {
    setFilteredPublications(papers.filter((pub) => pub.type === currentType));
  }, [currentType]);

  const handleTypeChange = (type) => setCurrentType(type);

  return (
    <div className="flex-grow sm:flex-row flex-col flex bg-primary w-full">
      ...
      <SideNav
        title="Publications"
        navList={getPublicationTypes(papers)}
